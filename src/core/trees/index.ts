...
export function bstInsert(root: BSTNode | null, val: number, steps: any[] = []): { root: BSTNode; steps: any[] } {
  if (!root) {
  ...
  const path: number[] = [];
  function ins(node: BSTNode, val: number) {
    path.push(node.val);
    steps.push({ type: 'compare', comparing: node.val, val, path: [...path] });
    if (val < node.val) {
      if (!node.left) {
        node.left = new BSTNode(val);
        steps.push({ type: 'placed', val, parent: node.val, side: 'left', path: [...path] });
      } else ins(node.left, val);
    } else {
      if (!node.right) {
        node.right = new BSTNode(val);
        steps.push({ type: 'placed', val, parent: node.val, side: 'right', path: [...path] });
...
export function bstDelete(root: BSTNode | null, val: number, steps: any[] = []): { root: BSTNode | null; steps: any[] } {
  function minVal(node: BSTNode): number {
...
export function treeToLayout(root: BSTNode | null, W: number = 640): { nodes: any[]; edges: any[] } {
  if (!root) return { nodes: [], edges: [] };
  const nodes: any[] = [];
  const edges: any[] = [];
  function traverse(node: BSTNode | null, x: number, y: number, level: number, xMin: number, xMax: number) {
    if (!node) return;
...
export function avlInsert(root: BSTNode | null, val: number, steps: any[] = []): { root: BSTNode; steps: any[] } {
  function height(n: BSTNode | null): number {
    return n ? n.h : 0;
  }
  function bf(n: BSTNode | null): number {
    return n ? height(n.left) - height(n.right) : 0;
  }
  function upd(n: BSTNode | null) {
    if (n) n.h = 1 + Math.max(height(n.left), height(n.right));
  }

  function rotR(y: BSTNode): BSTNode {
    let x = y.left!;
    let T2 = x.right;
  ...
  function rotL(x: BSTNode): BSTNode {
    let y = x.right!;
    let T2 = y.left;
  ...
  function ins(node: BSTNode | null, val: number): BSTNode {
    if (!node) {
      const n = new BSTNode(val);
      n.h = 1;
      steps.push({ type: 'placed', val });
      return n;
    }
...
export function bstFloor(root: BSTNode | null, key: number, steps: any[] = []): { floor: number | null; steps: any[] } {
  let floor: number | null = null;
  function search(node: BSTNode | null) {
    if (!node) return;
    steps.push({
      type: 'visit',
      node: node.val,
      key,
      floor,
      desc: `Xét ${node.val}: ${node.val <= key ? `${node.val} ≤ ${key}, cập nhật floor` : `${node.val} > ${key}, đi trái`}`
    });
    if (node.val === key) {
      floor = node.val;
  ...
  steps.push({
    ...
    floor,
    key,
    desc: floor !== null ? `✓ Floor(${key}) = ${floor}` : `✗ Không có floor cho ${key}`
  });
  return { floor, steps };
}

export function bstCeil(root: BSTNode | null, key: number, steps: any[] = []): { ceil: number | null; steps: any[] } {
  ...
  function search(node: BSTNode | null) {
    ...
    steps.push({
      ...
      key,
      ceil,
      desc: `Xét ${node.val}: ${node.val >= key ? `${node.val} ≥ ${key}, cập nhật ceil` : `${node.val} < ${key}, đi phải`}`
    });
    if (node.val === key) {
      ceil = node.val;
  ...
  steps.push({
    ...
    ceil,
    key,
    desc: ceil !== null ? `✓ Ceil(${key}) = ${ceil}` : `✗ Không có ceil cho ${key}`
  });
  return { ceil, steps };
}