// core/knowledge/index.js

// ─── 1. FRAME SYSTEM ──────────────────────────────────────────────────────────
export function frameOps(operations) {
  const steps = [];
  const frames = {}; // Lưu trữ tất cả frames
  const inheritanceMap = {}; // Lưu quan hệ kế thừa

  // Hàm tìm kiếm kế thừa từ một frame
  function getInheritanceChain(frameName) {
    const chain = [];
  ...
  // Hàm lấy tất cả slots từ frame (bao gồm kế thừa)
  function getAllSlots(frameName) {
    const allSlots = {};
...
// ─── 2. CONCEPT GRAPH ────────────────────────────────────────────────────────
export function conceptGraphOps(graph, operations) {
  const steps = [];
  ...
  function findPath(from, to, relation = null) {
  ...
  function findAncestors(node, relation = 'is-a') {
    const ancestors = [];
    const queue = [node];
    const visited = new Set();
    while (queue.length > 0) {
  ...
  function findDescendants(node, relation = 'is-a') {
    const descendants = [];
    const queue = [node];
    const visited = new Set();
    while (queue.length > 0) {
      ...
      if (visited.has(current)) continue;
      visited.add(current);
      const children = reverseAdj[current] || []; // Sửa chiều duyệt cho chuẩn cây phân cấp ngược
      ...
      if (ancestors.length > 0) {
        for (let i = 0; i < ancestors.length; i++) {
          steps.push({
            type: 'ancestor_step',
            node: op.node,
            // Sửa đổi quan trọng: đồng bộ mảng ancestors cho UI nhận diện và highlight
...
// ─── 3. SEMANTIC NETWORK ────────────────────────────────────────────────────
export function semanticNetOps(network, operations) {
  const steps = [];
  const { concepts = [], relations = [], instances = [] } = network;
  const activation = {};
  concepts.forEach(c => activation[c] = 0);

  function spreadActivation(node, initialStrength = 1.0, decay = 0.5, maxHops = 3) {
    const localActivation = {};
    concepts.forEach(c => localActivation[c] = 0);

    const visited = new Set();
    const queue = [{ node, strength: initialStrength, hops: 0 }];
    const result = [];
    ...
    while (queue.length > 0) {
      ...
      const neighbors = [];
      relations.forEach(r => {
        if (r.from === current.node) neighbors.push({ to: r.to, label: r.label });
        if (r.to === current.node) neighbors.push({ to: r.from, label: r.label });
      });
  ...
  for (const op of operations) {
    if (op.type === 'add_concept') {
      ...
      for (let i = 0; i < spreadResult.length; i++) {
        ...
        steps.push({
          ...
          relations: [...relations],
          instances: [...instances],
          // Sửa đổi quan trọng: Ép nhận snapshot độc lập theo dòng thời gian lan truyền
    ...
    // ... Giữ nguyên các phần 'infer' và 'query' cũ bên dưới ...
  }
