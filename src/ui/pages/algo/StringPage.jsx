...
export default function StringPage() {
  ...
  function runSteps(s, actionName) {
    ...
    const eng = new AnimationEngine({
      ...
      onDone: () => { setPlaying(false); saveProgress('string', actionName ?? ALGOS[algo]?.name); },
    });
  ...
  function runSearch() {
    ...
    runSteps(s, `${ALGOS[algo].name} - "${patInput}" trong "${textInput}"`);
  }
  ...
  function runRadix() {
    ...
    runSteps(s, ALGOS[algo].name);
  }

  function run3Way() {
    const words = radixInput.trim().split(/\s+/).filter(Boolean);
    if (!words.length) return;
    runSteps(radixQuick3Way(words), ALGOS.radix3.name);
  }

  function runSuffix() {
    if (!suffixInput.trim()) return;
    runSteps(buildSuffixArray(suffixInput.trim()), `${ALGOS.suffix.name} - "${suffixInput.trim()}"`);
  }
  ...
  function handleTstClear() {
    ...
    setTstWords([]);
    setTstResult(null);
    setSteps([]); setCurStep(null); setStepIdx(0);
    setPlaying(false); engineRef.current?.pause();
  }
  ...
  function stepDesc(s) {
    ...
    if (s.type === 'compare')       return `So sánh text[${s.ti}]='${s.char_t}' với pattern[${s.pi}]='${s.char_p}' ${s.match === false ? '✗' : s.match === true ? '✓' : ''}`;
    ...
    if (s.type === 'init' && (algo === 'lsd' || algo === 'msd')) return `Khởi tạo: ${s.arr?.length} từ, W=${s.W}`;
      ...
      <div className="algo-tabs" style={{ flexWrap: 'wrap' }}>
        {Object.entries(ALGOS).map(([k, v]) => (
          <button key={k} className={`algo-tab ${algo === k ? 'active' : ''}`}
            ...
            onClick={() => {
              setAlgo(k); setSteps([]); setCurStep(null); setTstResult(null);
              setStepIdx(0); setPlaying(false); engineRef.current?.pause();
            }}>
            {v.name}
            ...
            <div className="match-viz">
              ...
              <div className="match-pat-row">
                ...
                {patInput.split('').map((ch, i) => (
                  <div key={i} className={`match-char pattern-char ${curPi === i ? 'active' : ''}`}>
                    <span className="mc-idx">{i}</span>
                    <span className="mc-ch">{ch}</span>
                  </div>
                ))}
                ...
                <div className="hash-display">
                  {curStep?.type === 'roll' && (
                    <span style={{ color: '#a78bfa' }}>
                      Rolling: bỏ '{curStep.removed}', thêm '{curStep.added}'
                    </span>
                  )}
                  {curStep?.hash !== undefined && curStep?.type !== 'roll' && (
                    <>
                      Hash hiện tại: <b style={{ color: '#58a6ff' }}>{curStep.hash}</b>
              ...
              {matches.length > 0 && (
                <div className="match-result">
                  ✓ Tìm thấy tại vị trí: {matches.map(m => <span key={m} className="match-pos">{m}</span>)}
                </div>
              )}
            </div>
          )}
          ...
          {isRadixAlgo && (
            <div className="radix-viz">
              ...
              <div className="radix-words">
                {curArr.map((w, i) => {
                  const highlighted = curStep?.word === w;
              ...
              {curStep?.type === 'done' && sortedArr.length > 0 && (
                <div className="radix-result">Kết quả: {sortedArr.join(' → ')}</div>
              )}
            </div>
          )}
            ...
            <div className="tst-viz">
              {tstWords.length > 0 && (
                <div style={{ fontSize: 11, color: '#4a6b8a', marginBottom: 6 }}>
                  Từ đã chèn: {tstWords.map(w => <span key={w} className="tst-word-tag">{w}</span>)}
                </div>
              )}
        ...
        <div className="string-sidebar">
          {isSearchAlgo && (
            <div className="ctrl-section">
              ...
              <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 3 }}>
                <button className="btn-random" style={{ fontSize: 10 }} onClick={() => { setTextInput('AABAACAADAABAABA'); setPatInput('AABA'); }}>Ví dụ AABA</button>
                <button className="btn-random" style={{ fontSize: 10 }} onClick={() => { setTextInput('the quick brown fox jumps over the lazy dog'); setPatInput('the'); }}>Ví dụ "the"</button>
              </div>
            </div>
          )}
          ...
          {isRadixAlgo && (
            <div className="ctrl-section">
              ...
              <button className="btn-random" style={{ marginTop: 4, width: '100%', fontSize: 10 }}
                onClick={() => setRadixInput('she sells sea shells at the shore')}>Ví dụ "she sells sea..."</button>
          ...
          {isTST && (
            <div className="ctrl-section">
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                <button className="btn-random" style={{ flex: 1, fontSize: 10 }} onClick={() => {
                ...
                <button className="btn-danger" style={{ flex: 1, fontSize: 10 }} onClick={handleTstClear}>
                  🗑 Xóa TST
                </button>
              </div>
            </div>
          ...
          {isSuffix && (
            <div className="ctrl-section">
              ...
              <input className="arr-input" style={{ width: '100%', marginTop: 4 }}
                value={suffixInput} onChange={e => setSuffixInput(e.target.value)} placeholder="mississippi" />
          ...
          <div className="ctrl-section">
            ...
            <div style={{ fontSize: 11, color: '#4a6b8a', lineHeight: 1.7 }}>
              {algo === 'kmp'    && <><b style={{ color: '#8b9eb5' }}>KMP</b>: Dùng bảng LPS để bỏ qua so sánh thừa. O(n+m).</>}
              {algo === 'bm'     && <><b style={{ color: '#8b9eb5' }}>Boyer-Moore</b>: Dịch pattern dựa vào bad-character. Rất nhanh trong thực tế.</>}
              {algo === 'rk'     && <><b style={{ color: '#8b9eb5' }}>Rabin-Karp</b>: Rolling hash thật sự O(n). Dùng modular inverse để chia base.</>}
              {algo === 'radix3' && <><b style={{ color: '#8b9eb5' }}>3-Way Radix Quick</b>: Quicksort theo ký tự, partition 3 phần. O(W·N) worst.</>}
              {algo === 'suffix' && <><b style={{ color: '#8b9eb5' }}>Suffix Array</b>: Mảng hậu tố sắp xếp từ điển + LCP array. O(N log N).</>}
        ...
        <div className="radix-viz">
          ...
          <div className="radix-words">
            {(curStep?.arr || radixInput.trim().split(/\s+/)).map((w, i) => {
              const d = curStep?.d ?? 0;
              return (
                <div key={i} className={`radix-word ${(curStep?.lt === i || curStep?.gt === i) ? 'radix-active' : ''}`}>
                  {w.split('').map((ch, j) => (
                    <span key={j} className={`radix-ch ${j === d ? 'radix-focus' : ''}`}>{ch}</span>
                  ))}
                </div>
...
function TSTViz({ root, curStep }) {
  ...
  function traverse(node, x, y, spread) {
    ...
    if (node.left)  { edges.push({ x1: x, y1: y, x2: x - spread, y2: y + 55, label: '<' }); traverse(node.left,  x - spread, y + 55, ns); }
    if (node.mid)   { edges.push({ x1: x, y1: y, x2: x,          y2: y + 55, label: '=' }); traverse(node.mid,   x,          y + 55, ns); }
    if (node.right) { edges.push({ x1: x, y1: y, x2: x + spread, y2: y + 55, label: '>' }); traverse(node.right, x + spread, y + 55, ns); }
  }
    ...
    <svg width="100%" viewBox="0 0 600 320" style={{ background: '#0d1117', borderRadius: 8, border: '1px solid #1e2d3d' }}>
      ...
      {nodes.map((n, i) => (
        <g key={i}>
          ...
          {n.isEnd && <circle cx={n.x + 12} cy={n.y - 12} r={4} fill="#10b981" stroke="#0a0e1a" strokeWidth="1" />}
        </g>
      ))}
    </svg>
  );
